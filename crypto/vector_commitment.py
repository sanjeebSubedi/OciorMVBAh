...
if __name__ == "__main__":
    # Test with mock data to avoid circular dependency
    # For real testing with erasure coding, run: python -m crypto.test_integration
    test_data = [b"shard1", b"shard2", b"shard3", b"shard4"]

    # Create commitment
    commitment = vc_commit(test_data)
    print(f"Commitment: {commitment}")

    # Test proof for each shard
    for i in range(len(test_data)):
        shard, proof = vc_open(test_data, i)
